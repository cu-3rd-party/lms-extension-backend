...
class TestFullGetLongreadAPI(APITestCase):
    ...
    def setUp(self):
        # <--- ИЗМЕНЕНО: Добавляем аутентификацию
        create_and_authenticate_user(self.client)
        self.longread = Longread.objects.create(lms_id=1, title="Existing Longread", theme_id=2, course_id=3)
        self.longread.contents.save("test.pdf", ContentFile(b"TESTDATA"))

    def test_get_longread_contents_success(self):
        url = reverse("api-1.0.0:get_longread_contents", kwargs={"course_id": 3, "theme_id": 2, "longread_id": 1})
    ...
    def test_get_longread_contents_not_found(self):
        url = reverse("api-1.0.0:get_longread_contents", kwargs={"course_id": 99, "theme_id": 99, "longread_id": 99})
        response = self.client.get(url)
...
class TestGetCourseAPI(APITestCase):
    ...
    def setUp(self):
        ...
        create_and_authenticate_user(self.client)
        self.course_id = 42
        self.longread = Longread.objects.create(lms_id=2, title="Course Longread", theme_id=1, course_id=self.course_id)
...
class TestGetThemeAPI(APITestCase):
    ...
    def setUp(self):
        ...
        self.course_id = 50
        self.theme_id = 5
        self.longread = Longread.objects.create(lms_id=3, title="Theme Longread", theme_id=self.theme_id, course_id=self.course_id)

    def test_get_theme_success(self):
        url = reverse("api-1.0.0:get_theme", kwargs={"course_id": self.course_id, "theme_id": self.theme_id})
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_get_theme_not_found(self):
        url = reverse("api-1.0.0:get_theme", kwargs={"course_id": 999, "theme_id": 999})
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
...
class TestGetAvailableInfoAPI(APITestCase):
    ...
    def setUp(self):
        ...
        self.theme_id_2 = 5
        self.longreads = [
            Longread.objects.create(lms_id=1, title="test1", theme_id=self.theme_id_1, course_id=self.course_id_1),
            Longread.objects.create(lms_id=2, title="test2", theme_id=self.theme_id_1, course_id=self.course_id_1),
            Longread.objects.create(lms_id=3, title="test3", theme_id=self.theme_id_2, course_id=self.course_id_2),
        ]
...
class TestFetchLongreadsAPI(APITestCase):
    ...
    def setUp(self):
        ...
        self.valid_body = { "courses": [{"course_id": 1, "themes": [{"theme_id": 10, "longreads": [100, 101]}, {"theme_id": 11, "longreads": [102]}]}]}
    ...
    def test_fetch_with_all_missing_longreads(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertCountEqual(response.json()["missing_longreads"], [100, 101, 102])

    def test_fetch_with_some_existing_longreads(self):
        Longread.objects.create(lms_id=100, course_id=1, theme_id=10, title="Existing longread")
        response = self.client.post(self.url, self.valid_body, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_fetch_with_multiple_courses(self):
        body = {"courses": [{"course_id": 1, "themes": [{"theme_id": 10, "longreads": [200]}]}, {"course_id": 2, "themes": [{"theme_id": 20, "longreads": [300, 301]}]}]}
